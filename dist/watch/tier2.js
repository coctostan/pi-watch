 ...
 */
export function buildTier2Request(set, question, config) {
 ...
 */
export function parseTier2Answer(json) {
 ...
 */
export const TIER2_UNCONFIGURED_HINT = "Tier 2 (OpenAI-compatible sampled-frame vision) is unconfigured — set " +
 ...
 */
export function resolveTier2ConfigFromEnv(env = process.env) {
    const apiKey = env.WATCH_TIER2_API_KEY?.trim();
    const withApiKey = (base) => apiKey ? { ...base, apiKey } : base;
    const baseURL = env.WATCH_TIER2_BASE_URL?.trim();
    const model = env.WATCH_TIER2_MODEL?.trim();
    if (baseURL && model)
        return withApiKey({ baseURL, model });
...
export function createTier2Runner(deps) {
    ...
    return async ({ set, question }) => {
        const config = deps && "config" in deps ? deps.config : resolveTier2ConfigFromEnv();
        if (!config) {
        ...
        const requestInit = typeof timeoutMs === "number" && Number.isFinite(timeoutMs) && timeoutMs > 0
