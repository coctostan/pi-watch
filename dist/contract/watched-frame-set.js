...
// ── Enumerations ────────────────────────────────────────────────────────────
export const ResolutionTier = Type.Union([Type.Literal("low"), Type.Literal("high")], {
    $id: "ResolutionTier",
    description: "Frame resolution policy: 'low' by default; 'high' only for on-screen-text (OCR-ish) questions.",
});
export const FrameOrigin = Type.Union([Type.Literal("scene-cut"), Type.Literal("backfill")], {
    $id: "FrameOrigin",
    description: "How the frame was selected: 'scene-cut' (ffmpeg scene change) or 'backfill' (uniform gap fill).",
});
export const TranscriptSource = Type.Union([Type.Literal("captions"), Type.Literal("whisper")], {
    $id: "TranscriptSource",
    description: "Origin of transcript text: 'captions' (yt-dlp captions) or 'whisper' (local ASR).",
});
export const MediaType = Type.Union([Type.Literal("image/png"), Type.Literal("image/jpeg")], {
    $id: "MediaType",
...
export const WatchedFrame = Type.Object({
    ...
    timestamp: Type.String({
        description: "Human-readable mm:ss (or h:mm:ss) display offset, derived from tMs.",
    }),
...
export const TranscriptSegment = Type.Object({
    ...
    endMs: Type.Integer({
        minimum: 0,
        description: "Segment end offset on the shared timeline, in milliseconds (>= startMs).",
    }),
 ...
 */
export function validateWatchedFrameSet(value) {
    const errors = [];
    ...
    for (let i = 1; i < set.frames.length; i++) {
        ...
        if (cur.tMs < prev.tMs) {
            errors.push(`frames: not ordered by tMs ascending at index ${i} (${cur.tMs} < ${prev.tMs}).`);
        }
    }
    ...
    for (let i = 0; i < set.transcript.length; i++) {
        ...
        if (seg.endMs < seg.startMs) {
            errors.push(`transcript[${i}]: endMs (${seg.endMs}) < startMs (${seg.startMs}).`);
        }
        if (i > 0) {
            const prev = set.transcript[i - 1];
            if (seg.startMs < prev.startMs) {
                errors.push(`transcript: not ordered by startMs ascending at index ${i} (${seg.startMs} < ${prev.startMs}).`);
            }
        }
    }
...
//# sourceMappingURL=watched-frame-set.js.map