 ...
 */
function cadenceStep(sortedCuts) {
    const diffs = [];
 ...
 */
export function selectFrameTimes(opts) {
    ...
    const cuts = Array.from(new Set(opts.sceneCutsMs
        .map((c) => Math.floor(c))
        .filter((c) => Number.isFinite(c) && c >= 0 && c < durationMs))).sort((a, b) => a - b);
...
//# sourceMappingURL=select-frames.js.map