...
function parseUnsignedSeconds(value) {
    if (/^\d+$/.test(value)) {
...
function singletonTimestamp(params, key) {
...
export function parseYouTubeStartSeconds(ref) {
    let url;
    try {
        url = new URL(ref);
    }
    ...
    if (startValues.length > 0)
        return singletonTimestamp(url.searchParams, "start");
    return singletonTimestamp(url.searchParams, "t");
}
function explicitSeconds(name, value) {
    if (value === undefined)
        return undefined;
    if (!Number.isSafeInteger(value) || value < 0 || value > MAX_RANGE_SECONDS) {
        throw new Error(`Invalid watch range ${name}: expected a non-negative conversion-safe whole second integer no greater than ${MAX_RANGE_SECONDS}.`);
    }
...
export function resolveEvidenceRange(input) {
    ...
    const urlStart = input.urlStartSeconds !== undefined &&
...
export function clipTranscriptToRange(segments, range) {
    return segments
...
export function sceneCutsWithinRange(sceneCutsMs, range) {
    return sceneCutsMs
...
export function rebaseSelectedFrames(selected, range) {
    return selected.map((frame) => ({ ...frame, tMs: frame.tMs + range.startMs }));
}
const EMPTY_COVERAGE = { count: 0, firstMs: null, lastMs: null };
export function summarizeTranscriptCoverage(segments) {
...
export function summarizeFrameCoverage(frames) {
    if (frames.length === 0)
        return { ...EMPTY_COVERAGE };
    return {
        count: frames.length,
